# Audit Log Auto Tracking

## Purpose

Auto-tracking of audit log events across all system modules. Ensures every significant user action and system event is automatically recorded in the AuditLog table via `logAction()` or `logFailedAction()` utilities.
