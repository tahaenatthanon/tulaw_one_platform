...
The system SHALL display a prominent "Sign in with Microsoft" button on the login page (`/login`) that initiates the Microsoft Entra ID OAuth flow.
...
- **AND** the button SHALL redirect to `/api/auth/signin/microsoft-entra` when clicked
