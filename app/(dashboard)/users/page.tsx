...
import {
  ...
  GraduationCap, Building2, User, Eye, X, Lock, Key,
...
type TabId = "user-mgmt" | "role-mgmt" | "perm-mgmt" | "ad-sync";
...
const TABS: { id: TabId; label: string; icon: typeof Settings }[] = [
  ...
  { id: "perm-mgmt", label: "Permission Management", icon: Key },
...
const generateUsers = (count: number): UserData[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    users.push({ id: String(i + 1), name, email: `${englishName}@tulaw.ac.th`, role, department: rand(DEPARTMENTS), status: status as UserData["status"], lastAdSync: randDate(7) });
  }
  return users;
};
...
function AddUserDialog({ open, onClose, onAdd }: {
  ...
  const handle = () => {
    ...
    setName(""); setEmail(""); setRole("User"); setDept(DEPARTMENTS[0]); setStatus("active"); onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-tu-surface rounded-[--radius-dialog] border border-tu-border shadow-xl w-full max-w-md mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        <p className="text-sm text-tu-text-secondary mb-4">อัปโหลดไฟล์ CSV ที่มีคอลัมน์: Name, Email, Role, Department, Status</p>
...
function UserManagementTab({ users, setUsers }: { users: UserData[]; setUsers: (u: UserData[]) => void }) {
  ...
  const handleAdd = (u: Omit<UserData, "id" | "lastAdSync">) => {
    const now = new Date();
    setUsers([{ ...u, id: String(users.length + 1), lastAdSync: now.toISOString().slice(0, 10) + " " + now.toLocaleTimeString("th-TH", { hour: "2-digit", minute: "2-digit" }) }, ...users]);
  };
  const handleImport = () => setUsers([...Array.from({ length: 10 }, () => generateUsers(1)[0]), ...users]);

  const filtered = users.filter(u =>
    (u.name.toLowerCase().includes(search.toLowerCase()) || u.email.toLowerCase().includes(search.toLowerCase())) &&
    (!roleFilter || u.role === roleFilter) && (statusFilter === "all" || u.status === statusFilter)
  );

  const roleCounts = useMemo(() => { const c: Record<string, number> = {}; ROLES.forEach(r => c[r] = users.filter(u => u.role === r).length); return c; }, [users]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {ROLE_STATS.map(s => (<div key={s.role} className="bg-tu-surface rounded-[--radius-card] border border-tu-border p-3"><div className="flex items-center gap-2 mb-1"><div className={cn("flex h-7 w-7 items-center justify-center rounded-lg", s.bg)}><s.icon size={14} className={s.color} /></div><span className="text-xs font-medium text-tu-text-muted">{s.label}</span></div><p className="text-xl font-bold text-tu-text-primary tabular-nums">{roleCounts[s.role] || 0}</p></div>))}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)} className="rounded-[--radius-input] border border-tu-border bg-tu-surface px-3 py-2 text-sm outline-none focus:border-tu-border-focus focus:ring-2 focus:ring-tu-border-focus/20"><option value="">ทุกบทบาท</option>{ROLES.map(r => <option key={r} value={r}>{r}</option>)}</select>
        <div className="flex gap-1 bg-tu-surface border border-tu-border rounded-lg p-0.5">
          {[{ v: "all", l: "ทั้งหมด" }, { v: "active", l: "Active" }, { v: "inactive", l: "Inactive" }, { v: "mfa_pending", l: "MFA Pending" }].map(f => (<button key={f.v} onClick={() => setStatusFilter(f.v)} className={cn("rounded-md px-3 py-1.5 text-xs font-medium transition-colors", statusFilter === f.v ? "bg-tu-primary text-white shadow-sm" : "text-tu-text-secondary hover:text-tu-text-primary")}>{f.l}</button>))}
        </div>
      </div>
      ...
      {filtered.length > pageSize && (<div className="flex items-center justify-between"><span className="text-xs text-tu-text-muted">แสดง {(page-1)*pageSize+1}–{Math.min(page*pageSize,filtered.length)} จาก {filtered.length} รายการ</span><div className="flex items-center gap-1"><button onClick={()=>setPage(page-1)} disabled={page<=1} className="p-1.5 rounded-md text-tu-text-muted hover:bg-tu-surface-hover disabled:opacity-30"><ChevronLeft size={16}/></button>{(()=>{const pages:number[]=[];if(totalPages<=7)for(let i=1;i<=totalPages;i++)pages.push(i);else if(page<=4)pages.push(1,2,3,4,5,-1,totalPages);else if(page>=totalPages-3)pages.push(1,-1,totalPages-4,totalPages-3,totalPages-2,totalPages-1,totalPages);else pages.push(1,-1,page-1,page,page+1,-1,totalPages);return pages.map((n,i)=>n===-1?<span key={`e${i}`} className="px-1 text-tu-text-muted">…</span>:<button key={n} onClick={()=>setPage(n)} className={cn("px-2.5 py-1 rounded text-xs font-medium transition-colors",n===page?"bg-tu-primary text-white":"text-tu-text-secondary hover:bg-tu-surface-hover")}>{n}</button>);})()}<button onClick={()=>setPage(page+1)} disabled={page>=totalPages} className="p-1.5 rounded-md text-tu-text-muted hover:bg-tu-surface-hover disabled:opacity-30"><ChevronRight size={16}/></button></div></div>)}
...
function RoleManagementTab() {
  const roles = [
    { id: "1", name: "Super Admin", level: 100, users: 2, permissions: "ทั้งหมด" },
    { id: "2", name: "System Admin", level: 80, users: 8, permissions: "จัดการระบบ" },
    { id: "3", name: "Dean", level: 70, users: 5, permissions: "Dashboard, Reports" },
    { id: "4", name: "Dept Admin", level: 50, users: 45, permissions: "Department" },
    { id: "5", name: "User", level: 30, users: 210, permissions: "General" },
    { id: "6", name: "Viewer", level: 10, users: 45, permissions: "Read Only" },
  ];
  return (
    <div className="space-y-4">
      <div className="bg-tu-surface rounded-[--radius-card] border border-tu-border overflow-auto">
        <table className="w-full min-w-[600px]">
          <thead><tr className="bg-tu-bg border-b border-tu-border text-left"><th className="px-4 py-3 text-xs font-semibold text-tu-text-secondary uppercase">บทบาท</th><th className="px-4 py-3 text-xs font-semibold text-tu-text-secondary uppercase">Level</th><th className="px-4 py-3 text-xs font-semibold text-tu-text-secondary uppercase">จำนวนผู้ใช้</th><th className="px-4 py-3 text-xs font-semibold text-tu-text-secondary uppercase">สิทธิ์</th></tr></thead>
          <tbody className="divide-y divide-tu-border">
            {roles.map(r => (
              <tr key={r.id} className="hover:bg-tu-surface-hover transition-colors">
                <td className="px-4 py-3 text-sm font-medium text-tu-text-primary">{r.name}</td>
                <td className="px-4 py-3 text-sm text-tu-text-secondary">{r.level}</td>
                <td className="px-4 py-3 text-sm text-tu-text-secondary">{r.users}</td>
                <td className="px-4 py-3 text-sm text-tu-text-secondary">{r.permissions}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function PermissionManagementTab() {
  const perms = ["USERS_VIEW", "USERS_CREATE", "USERS_EDIT", "USERS_DELETE", "USERS_AD_SYNC", "USERS_BULK_IMPORT", "AUDIT_LOG_VIEW", "AUDIT_LOG_EXPORT", "SETTINGS_VIEW", "SETTINGS_MANAGE", "DASHBOARD_VIEW", "DASHBOARD_MANAGE", "INTRANET_VIEW", "INTRANET_CREATE"];
  const roles = ["Super Admin", "System Admin", "Dean", "Dept Admin", "User", "Viewer"];
  return (
    <div className="space-y-4">
      <div className="bg-tu-surface rounded-[--radius-card] border border-tu-border overflow-auto">
        <table className="w-full min-w-[800px]">
          <thead><tr className="bg-tu-bg border-b border-tu-border text-left"><th className="px-4 py-3 text-xs font-semibold text-tu-text-secondary uppercase">Permission</th>{roles.map(r => <th key={r} className="px-4 py-3 text-xs font-semibold text-tu-text-secondary uppercase text-center">{r}</th>)}</tr></thead>
          <tbody className="divide-y divide-tu-border">
            {perms.map(p => (
              <tr key={p} className="hover:bg-tu-surface-hover transition-colors">
                <td className="px-4 py-3 text-xs font-mono text-tu-text-secondary">{p}</td>
                {roles.map((_, i) => <td key={i} className="px-4 py-3 text-center">{i <= 1 ? "✅" : i <= 3 ? (i <= 2 ? "✅" : "◐") : i === 4 ? (p.startsWith("USERS_") ? "❌" : "✅") : "❌"}</td>)}
              </tr>
            ))}
          </tbody>
        </table>
...
function AdSyncTab() {
  const [syncing, setSyncing] = useState(false);
  const logs = [
    { id: "1", time: "2025-07-13 14:30", status: "success", detail: "ซิงค์สำเร็จ — 315 users" },
    { id: "2", time: "2025-07-13 14:15", status: "success", detail: "ซิงค์สำเร็จ — 313 users" },
    { id: "3", time: "2025-07-13 14:00", status: "success", detail: "ซิงค์สำเร็จ — 312 users" },
  ];
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        <button onClick={async () => { setSyncing(true); await new Promise(r => setTimeout(r, 2000)); setSyncing(false); }} disabled={syncing} className="flex items-center gap-1.5 rounded-[--radius-btn] bg-tu-primary px-4 py-2 text-sm font-medium text-white hover:bg-tu-primary-hover transition-colors disabled:opacity-50"><RefreshCw size={16} className={syncing ? "animate-spin" : ""} />{syncing ? "กำลังซิงค์..." : "Sync Now"}</button>
        <span className="text-xs text-tu-text-muted">ซิงค์อัตโนมัติทุก 15 นาที</span>
      </div>
      <div className="bg-tu-surface rounded-[--radius-card] border border-tu-border p-4 space-y-3">
        <h3 className="text-sm font-semibold text-tu-text-primary">ประวัติการซิงค์</h3>
        {logs.map(l => (
          <div key={l.id} className="flex items-center gap-3 text-sm">
            <span className={cn("h-2 w-2 rounded-full", l.status === "success" ? "bg-tu-success" : "bg-tu-error")} />
            <span className="text-xs text-tu-text-muted">{l.time}</span>
            <span className="text-tu-text-secondary">{l.detail}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function UsersPage() {
  const [activeTab, setActiveTab] = useState<TabId>("user-mgmt");
  const [users, setUsers] = useState(MOCK_USERS);

  return (
    <div className="p-6 space-y-5">
      <div>
        <h1 className="text-2xl font-semibold text-tu-text-primary">Users & Roles</h1>
        <p className="text-tu-text-muted text-sm mt-1">จัดการผู้ใช้งาน — {users.length} บัญชี</p>
      </div>
      ...
      {activeTab === "user-mgmt" && <UserManagementTab users={users} setUsers={setUsers} />}
      {activeTab === "role-mgmt" && <RoleManagementTab />}
      {activeTab === "perm-mgmt" && <PermissionManagementTab />}
      {activeTab === "ad-sync" && <AdSyncTab />}
    </div>
  );
}
